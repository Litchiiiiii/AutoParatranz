...
class Term(BaseModel):
    ...
    updated_by: Optional[StrictInt] = Field(default=None, description="最后修改用户的ID", alias="updatedBy")
    pos: Optional[StrictStr] = Field(default=None, description="术语词性")
    uid: Optional[StrictInt] = Field(default=None, description="创建用户的ID")
    term: Optional[StrictStr] = Field(default=None, description="术语原文")
    translation: Optional[StrictStr] = Field(default=None, description="术语译文")
    note: Optional[StrictStr] = Field(default=None, description="术语注释")
    project: Optional[StrictInt] = Field(default=None, description="术语所属项目ID")
    variants: Optional[List[StrictStr]] = Field(default=None, description="术语原文的其他形式（复数形式、其他时态等）")
    __properties: ClassVar[List[str]] = ["id", "createdAt", "updatedAt", "updatedBy", "pos", "uid", "term", "translation", "note", "project", "variants"]

    @field_validator('pos')
    def pos_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in ('noun', 'verb', 'adj', 'adv'):
            raise ValueError("must be one of enum values ('noun', 'verb', 'adj', 'adv')")
        return value
