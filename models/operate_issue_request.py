...
class OperateIssueRequest(BaseModel):
    """
    OperateIssueRequest
    """ # noqa: E501
    op: StrictStr = Field(description="操作类型，回复为 `reply`，订阅为 `subscribe`，取消订阅为 `unsubscribe`")
    content: Optional[StrictStr] = Field(default=None, description="回复内容，支持markdown，仅 op 为 reply 时需要传入")
    __properties: ClassVar[List[str]] = ["op", "content"]

    @field_validator('op')
    def op_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('reply', 'subscribe', 'unsubscribe'):
            raise ValueError("must be one of enum values ('reply', 'subscribe', 'unsubscribe')")
        return value
